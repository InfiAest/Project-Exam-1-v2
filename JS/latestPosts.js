...
function nextPost(x) {
    showPosts(pageNumber += x);
};

function currentPost(x) {
    showPosts(pageNumber = x);
};

function showPosts(x) {
    const postSlides = document.querySelectorAll(".postSlide");
    const pageMarkers = document.querySelectorAll(".page-marker");

    if (x > postSlides.length) {
        pageNumber = 1
    };

    if (x < 1) {
        pageNumber = postSlides.length;
    };

    for (var i = 0; i < postSlides.length; i++) {
        postSlides[i].style.display = "none";  
    };

    for (i = 0; i < pageMarkers.length; i++) {
        pageMarkers[i].className = pageMarkers[i].className.replace(" active", "");
    };
  
    postSlides[pageNumber-1].style.display = "block";  
    pageMarkers[pageNumber-1].className += " active";
};
